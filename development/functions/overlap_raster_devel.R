...
#' } 
overlap_raster_devel <- function(species_name, 
                                 predictor,
                                 model = c("glm", "svm", "maxent")) {
  if (!require(raster)) {
  ...
  insects_hosts <- read.csv(file = "development/data/insect-host-limited.csv")
  ...
  insect_dist_file <- paste0("development/output/distributions/",
                             nice_name, 
                             "-distribution-",
                             model,
                             "-",
  ...
  if (length(hosts) >= 1) {
    ...
    for (host_name in hosts) {
      host_nice_name <- tolower(x = gsub(pattern = " ",
                                         replacement = "_",
                                         x = host_name))
      host_dist_file <- paste0("development/output/distributions/",
                               host_nice_name,
                               "-distribution-",
                               model,
                               "-",
      ...
      if (file.exists(host_dist_file)) {
        ...
        # Crop raster to extent of insect prediction area
        host_pa_list[[host_nice_name]] <- crop(host_pa_list[[host_nice_name]],
                                               insect_pa)
      } else {
    ...
    # Make sure at least one host had a map, otherwise go no further, message, 
    # and return NULL
