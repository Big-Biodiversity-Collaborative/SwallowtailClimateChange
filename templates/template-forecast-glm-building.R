...
model_file <- paste0("output/models/", nice_name, "-model-glm-current.rds")
...
if (file.exists(model_file) & file.exists(obs_file)) {
  ...
  glm_model <- readRDS(file = model_file)
  ...
  forecast_pa <- forecast_probs > glm_model$thresh
  ...
  message(paste0("GLM model forecast predictions for ", species_name, 
