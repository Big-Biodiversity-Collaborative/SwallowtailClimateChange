...
**Predicted areas of suitability for two _Papilio_ species.** Examples of areas 
that are predicted to be climatically suitable for a swallowtail species and 
their respective host plants now (a, b) and in the mid-twenty-first century (c, 
d), as well as predicted changes in swallowtail distributions between now and 
the mid-twenty-first century (e, f). Predictions for _Papilio indra_ are in the 
left column and predictions for _P. cresphontes_ are in the right column. 
Current distributions of butterflies are a consequence of mainly host suitable 
habitat while future distributions of both _Papilio_ species  are expected to 
expand in some areas and contract in other areas due to both climatic and host-
plant distributions. Loss of suitable area may occur when a region is not 
predicted to be climatically suitable for the swallowtail in the future (e.g., 
_P. indra_ in California) or because an area is not predicted to be 
climatically suitable for any of the swallowtail’s host plants (_P. cresphontes_
in Florida).
...
Percentage currently suitable area retained (left) and net change in total 
suitable area (right) in the SSP3-7.0 model in the 2050s.
:::
...
![](../output/manuscript/Figure-4-Lats-Hosts.png)

**Predicted changes in range position and relationship of diet to area changes.**
(A) Anticipated changes to range margins at southern latitudes (horizontal axis) and 
northern latitudes (vertical axis). Points to the right or below origin indicate 
contraction, while points to the right or above origin indicate expansion. (B) 
Association (add stats) between host diet breadth (number of host plant species) 
and predicted change in total suitable area.
...
![](../output/manuscript/Figure-Protected-Areas.png)

**Nationally protected areas and predicted change for different levels of protection.**
(A) Areas with national-level protection are concentrated in the western half 
of North America. (B) Suitability hotspots are predicted to increase in area on 
lands currently under national-level protection, while suitability hotspot 
areas are predicted to decrease on lands under the other three protection 
levels.
