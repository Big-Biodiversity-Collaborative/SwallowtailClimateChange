# Create a script for tuning models and CV model evaluation for each species
# Jeff Oliver
...
do
  ...
  FILENAME="src/indiv/${GENUSLOWER}_${SPECIES}-${SCRIPTTYPE}.R"
  
  # Read in the template file contents into the SCRIPTBODY variable, 
  # skipping first two lines
  TEMPLATE="templates/template-${SCRIPTTYPE}.R"
  SCRIPTBODY=$(cat "$TEMPLATE" | tail -n +2)
  
  # Add lines at top of file explaining script & message warning against 
  # editing
  MESSAGE="# This file is auto-generated by build-scripts-01-${SCRIPTTYPE}.sh. Do NOT edit."
  SCRIPTBODY=$(echo -e "# Generate ${PRODUCT} for\n# ${GENUS} ${SPECIES}\n${MESSAGE}\n${SCRIPTBODY}")
